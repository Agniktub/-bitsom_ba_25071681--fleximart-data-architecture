...
WITH ROLLUP
HAVING (year IS NOT NULL OR quarter IS NOT NULL OR month_name IS NOT NULL)
ORDER BY d.year, d.quarter, d.month;

-- Alternative without ROLLUP (explicit hierarchy)
-- SELECT 
--     d.year,
--     COALESCE(d.quarter, 'Total') AS quarter,
--     COALESCE(d.month_name, 'Total') AS month_name,
--     SUM(f.total_amount) AS total_sales,
--     SUM(f.quantity_sold) AS total_quantity
-- FROM fact_sales f
-- JOIN dim_date d ON f.date_key = d.date_key
-- GROUP BY GROUPING SETS ((d.year), (d.year, d.quarter), (d.year, d.quarter, d.month_name));
...
SELECT 
    ...
    ROUND(SUM(f.total_amount) / total_rev.overall_revenue * 100, 2) AS revenue_percentage
FROM fact_sales f
JOIN dim_product p ON f.product_key = p.product_key
CROSS JOIN (SELECT SUM(total_amount) AS overall_revenue FROM fact_sales) total_rev
GROUP BY p.product_key, p.product_name, p.category, total_rev.overall_revenue
